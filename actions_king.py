...
def actions_king(M,i,j,n_rows,n_col,escape_value):
    ...
    #move up:
    up= -1
    while i+up >= 0 and (M[i + up, j ] == 0 or M[i + up, j ] == escape_value):
       list_actions.append([[i,j],[i + up, j]])
       up -= 1

    # move down:
    down = +1
    while i+down < n_rows and (M[i + down , j] == 0 or M[i + down,j] == escape_value):
        list_actions.append([[i,j],[i + down , j]])
        down += 1

    # move left:
    left = -1
    while j+left >= 0 and (M[i , j + left] == 0 or M[i , j + left] == escape_value):
       list_actions.append([[i,j],[i , j + left]])
       left -= 1

    # move down:
    right = +1
    while j+right < n_col and (M[i , j + right] == 0 or M[i , j + right] == escape_value):
